...
`music.netease.showcase@2` corrects the showcase boundary: it is an explicitly ordered, server-bounded gallery of at most six selected resources and never substitutes a weekly/all-time rank when empty. `music.netease.ranking@2` contains separately scoped weekly and all-time payloads so one Renderer can switch views without a waterfall. Existing current v1 Widgets are upgraded by cloning immutable Revisions; historical v1 snapshots stay intact. Nivalis-composed galleries remain semantically distinct from the Provider-native personal-home arrangement. See ADR 0019 and its native-source follow-up ADR 0020.

The native arrangement is now independently available through the paginated NetEase profile-page read model. The first `/api/personal/home/page/user` request omits cursor; `hasMore=true` responses drive bounded follow-up pages whose exact Provider cursors and payloads are stored as independent `netease.profile_showcase[.page.N]` Raw evidence. The Normalizer merges music-taste, representative-song, album-rack, playlist, and `PERSONAL_SHOWCASE` resources in Provider order, using `uiElement.type` before broader resource/block heuristics. The Owner Catalog retains the complete normalized set, while Provider-mode publication excludes Provider-private cards and remains bounded to six; `custom` uses explicit Owner selections. Older Raw runs remain replayable without the new source. See ADR 0020.
