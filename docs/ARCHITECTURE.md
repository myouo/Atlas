...
The native arrangement is now independently available through the NetEase profile-page read model. The Connector stores `/api/personal/home/page/user` as `netease.profile_showcase`; the Normalizer selects only `PERSONAL_SHOWCASE`, validates the four known creative variants, removes the add-button affordance, and preserves at most six Provider-ordered cards. Showcase mode `provider` follows this data, while `custom` uses explicit Owner selections. Older Raw runs remain replayable without the new source. See ADR 0020.
